...
function chatAnswer(text,cb){
  ...
  'ТВОЯ БАЗА ЗНАНИЙ (опирайся на неё, бери ТОЛЬКО подходящее к вопросу, НИКОГДА не пересказывай списком и не цитируй целиком):\n'+
  (window.DZEN_KB||'')+'\n'+
  ...
  var bye=/(^|\s)(пока|всё понятно|все понятно|всё ясно|все ясно|хватит|достаточно|это всё|это все|больше ничего|ничего не нужно|ничего больше|до завтра|до встречи|бывай|отбой|спокойной ночи|закончим|конец|выход|стоп)(\s|[.,!]|$)/i.test(text)
    ||/^(спасибо|спасибки|благодарю|спс)[\s.,!)]*$/i.test(text.trim())
    ||/(спасибо|благодарю)[\s,]*(всё|все|достаточно|понятно|пока|до завтра)/i.test(text);
