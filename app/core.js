...
function llm(sys,user,cb){ /* user: строка или массив [{role,content}] ; cb(text|null) */
  ...
  if(c.provider==='claude'&&c.key){
    ...
    /* gpt-oss-120b заметно умнее на вопросах про сон/энергию и держит запрет банальностей;
       llama-3.3 — запасная (llama-4-maverick у Groq больше не существует, каждый запрос падал в 404) */
    var models=['openai/gpt-oss-120b','llama-3.3-70b-versatile'];
    /* Лимит бесплатного тарифа — 8000 токенов в минуту на модель. При всплеске
       Groq отвечает 429 и САМ пишет, через сколько можно повторить. Раньше код
       на 429 сразу прыгал на запасную модель, ловил 429 и там — и человек видел
       «связь моргнула». Теперь: ждём указанную паузу и повторяем, и только
       если и это не помогло — идём на запасную модель. */
    (function tryModel(mi,retried){
      ...
      fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Authorization':'Bearer '+c.key,'Content-Type':'application/json'},
        ...
        .then(function(o){
          var t=o.d.choices&&o.d.choices[0]?o.d.choices[0].message.content:null;
          if(t){cb(t);return;}
          if(o.s===429&&!retried){
            var msg=(o.d.error&&o.d.error.message)||'';
            var m=msg.match(/try again in ([\d.]+)s/i);
            var wait=Math.min(9000,Math.round((m?parseFloat(m[1]):3)*1000)+400);
            setTimeout(function(){tryModel(mi,true);},wait);return;}
          tryModel(mi+1,false);})
        .catch(function(){tryModel(mi+1,false);});
    })(0,false);}
  else cb(null);}
...
/* Ответы без нейронки: если Groq не ответил, человек всё равно получает дело,
   а не «связь моргнула». Порядок важен — сверху самое срочное (здоровье). */
var FAQ=[
 [/(перестаю дышать|не дышу|остановк\w* дыхан|апноэ|задыхаюсь во сне)/i,
  'Остановки дыхания во сне — это к врачу, не ко мне: похоже на апноэ, и само оно не проходит. Запишись к сомнологу или хотя бы терапевту. Пока ждёшь приёма: спи на боку, а не на спине, и не пей алкоголь вечером — оба пункта уменьшают остановки.'],
 [/(храп)/i,'Храп с паузами в дыхании стоит показать врачу — это может быть апноэ. Что помогает уже сегодня: сон на боку вместо спины и никакого алкоголя за 3-4 часа до сна.'],
 [/(депресс|не хочу жить|нет сил жить|всё бессмысленно)/i,
  'Это уже не про режим дня — с таким состоянием правильнее к врачу или психологу, и это нормальный шаг, а не слабость. Если тяжело прямо сейчас, помощь есть круглосуточно по телефону 8-800-2000-122.'],
 [/(не могу уснуть|бессонниц|засып|уснуть)/i,
  'Если не спится: не лежи в кровати дольше 20 минут — встань, побудь при тусклом свете и вернись, когда потянет в сон. Помогает выдох длиннее вдоха: 4 секунды вдох, 6-8 выдох, пара минут. И запиши 3 дела на завтра — мозг перестанет их «держать».'],
 [/(кофе|кофеин|энергетик)/i,
  'Кофеин выводится примерно 5-6 часов, так что чашка в 15:00 — это половина дозы в крови к 21:00. Последнюю пей за 8-10 часов до сна, а первую — не сразу после подъёма, а через час: так меньше дневной провал.'],
 [/(сладк|сахар в еде|тянет на|переедаю)/i,
  'Тяга к сладкому днём — почти всегда хвост от завтрака из быстрых углеводов. Попробуй 25-30 г белка утром (три яйца, творог, скир) — тяга к 15:00 обычно падает сама, без силы воли.'],
 [/(не высыпа|мало сплю|недосып|устал|нет сил|разбит)/i,
  'Начни не с длины сна, а с постоянства: разброс времени отбоя больше получаса бьёт сильнее, чем минус час сна. И утром 10 минут на улице в первые полчаса после подъёма — это самый сильный бесплатный рычаг.'],
 [/(спорт|зал|тренир|движени)/i,
  'Для энергии регулярность важнее интенсивности: 10 минут ходьбы бодрят сильнее чашки кофе и без отката. При недосыпе тяжёлая тренировка только навредит — лучше прогулка после еды.'],
 [/(стресс|нерв|тревог|напряж)/i,
  'Быстрый способ сбросить напряжение: выдох длиннее вдоха — 4 секунды вдох носом, 6-8 выдох ртом, две-три минуты. Работает за пару минут, потому что успокаивает через нервную систему, а не через уговоры.'],
 [/(алкогол|вино|пиво|выпил)/i,
  'Алкоголь помогает уснуть и ломает вторую половину ночи: подавляет глубокий сон и будит в 3-5 утра. Если пьёшь — то за 3-4 часа до сна и не каждый день.'],
...
function chatAnswer(text,cb){
  ...
  'ТВОЯ БАЗА ЗНАНИЙ (опирайся на неё, НИКОГДА не пересказывай списком и не цитируй целиком):\n'+
  (window.pickKB?pickKB(text,CHAT_HIST.map(function(m){return m.content;}).join(' ')):'')+'\n'+
  'Если вопрос выходит за эту базу — отвечай осторожно и честно говори, чего не знаешь, вместо выдумывания цифр.\n'+
  'ЗАПРЕЩЁННЫЕ ПУСТЫШКИ (не советуй никогда, это народные ритуалы без эффекта на энергию): вода с лимоном, детокс, «разгонять метаболизм», контрастный душ как замена сну, витамины «для бодрости» без анализов, «просто больше отдыхай». Если нечего сказать по делу — лучше задай уточняющий вопрос.\n'+
  'ЭМОДЗИ: допустимы только спокойные и в тему (🌿 🥤 ⚡ ◷). НИКАКИХ 🚀💪🔥✨🎯 и подобной бодрой мишуры. Чаще всего — вообще без эмодзи.\n'+
  phase+'\n'+
  ...
  /* если человек сейчас внутри опроса — нейронка ОБЯЗАНА держать тему вопроса,
     иначе на «всегда» в вопросе про сладкое она отвечала про сон */
  (function(){var pc=window.probeContext&&probeContext();if(!pc)return '';
    return '\n\nВАЖНО — СЕЙЧАС ИДЁТ ОПРОС. Человеку только что задали вопрос: «'+pc.q+'» '+
      '(варианты ответа: '+pc.opts.join(' / ')+'). Его сообщение — это реплика ИМЕННО НА ЭТОТ ВОПРОС. '+
      'Отвечай строго в теме этого вопроса, не переводи разговор на другую тему. '+
      'Если из его слов понятно, какой вариант он имел в виду — коротко подтверди это и объясни, что его ответ значит. '+
      'Если непонятно — одной строкой переспроси, уточнив варианты. Максимум 3 строки.';})();
  /* ЯВНОЕ ПРОЩАНИЕ важнее офтопа: человек сказал «всё, хватит» — закрываем сразу,
     иначе «спасибо» уходило в офтоп, сбрасывало счётчик и разговор не кончался никогда */
  var bye=(function(t){
    t=t.trim().toLowerCase().replace(/ё/g,'е');
    if(t.length>90)return false;                       /* длинный текст — это рассказ, не прощание */
    if(/\?$/.test(t))return false;                     /* вопрос закрывать нельзя */
    /* короткая реплика целиком из благодарности/согласия = «мне хватит» */
    if(/^(спасибо|спасибки|спс|благодарю|понял|поняла|понятно|ясно|ок|окей|ok|хорошо|принято|учту|попробую|буду|давай|угу|ага)[\s.,!)…]*$/.test(t))return true;
    /* явные прощания и просьбы закончить — в любом месте фразы */
    /* «пока» многозначно («пока что не очень») — принимаем только как отдельную реплику */
    if(/^(пока|покеда|пока-пока|ну пока|все пока|всё пока)[\s.,!…)]*$/.test(t))return true;
    if(/(^|[\s,.;!-])(бывай|прощай|до свидания|до завтра|до встречи|доброй ночи|спокойной ночи|хватит|достаточно|закончим|заканчиваем|закончили|отбой|отстань|надоел|надоело|я пошел|пошел я|ушел|отдыхай)([\s,.!…]|$)/.test(t))return true;
    /* «всё понял», «это всё», «больше ничего не надо» */
    if(/(^|\s)(все|это все|всё)([\s,]*(понял|поняла|понятно|ясно|спасибо|хватит|достаточно))?([\s,.!…]|$)/.test(t)
       &&/(понял|поняла|понятно|ясно|спасибо|хватит|достаточно|надоел)/.test(t))return true;
    if(/(больше )?(ничего|нечего)( больше)?( не)? *(надо|нужно|хочу|спрашива)/.test(t))return true;
    if(/(спасибо|благодарю|спс)/.test(t)&&/(все|всё|достаточно|понял|поняла|понятно|ясно|пока|до завтра|попробую|учту|буду|хорошо|ок)/.test(t))return true;
    return false;})(text);
  var offtop=!bye&&/(анекдот|шутк|пошути|смешн|прикол|спасибо|благодар|привет|как дела|ты кто|робот|человек)/i.test(text);
  if(bye){
    phase='ФАЗА «ПРОЩАНИЕ»: человек явно завершает разговор. Тепло попрощайся ОДНОЙ-ДВУМЯ строками. '+
      'СТРОГО ЗАПРЕЩЕНО: задавать любые вопросы (в ответе не должно быть знака «?»), давать новые советы, читать нотации, звать продолжить разговор. '+
      'Если по ходу разговора было задание — напомни его одной короткой фразой. Закончи приглашением заглянуть завтра. Максимум 2 строки.';}
  else if(offtop){
  ...
  llm(sys,CHAT_HIST.slice(),function(raw){
    if(raw){
      ...
      /* в прощании вопрос недопустим: модель иногда всё равно спрашивает —
         режем хвост с вопросом, чтобы разговор действительно закончился */
      if(bye&&a.indexOf('?')>=0){
        var keep=(a.match(/[^.!?…]+[.!…]+/g)||[]).filter(function(s){return s.indexOf('?')<0;}).join(' ').trim();
        a=keep||'Рад был помочь 🌿 Заходи завтра.';}
      CHAT_HIST.push({role:'assistant',content:a});
      if(CHAT_HIST.length>10)CHAT_HIST=CHAT_HIST.slice(-10);
      var closing=bye||(TURNS>=5&&!offtop);
      if(closing)LAST_TASK=a;
      cb(a,closing);return;}
    for(var i=0;i<FAQ.length;i++)if(FAQ[i][0].test(text)){cb(FAQ[i][1],false);return;}
    cb('Я сейчас думаю медленнее обычного — слишком много вопросов разом. Спроси ещё раз через полминуты, отвечу нормально. А пока: самый сильный ход для энергии — 10 минут на улице в первые полчаса после подъёма.',false);});}
